...
let run_lwt ?port ?timeout ?verifyurlhost ?certchains ?v4 ?v6 callback =
  ...
  in
  run ?port ?timeout ?verifyurlhost ?certchains stack callback

let run ?port ?timeout ?verifyurlhost ?certchains ?v4 ?v6 callback =
  run_lwt ?port ?timeout ?verifyurlhost ?certchains ?v4 ?v6 callback
  |> Lwt_main.run
